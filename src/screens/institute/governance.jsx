    ...
    <>
      <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'}`}>
        ...
        <div className="container mx-auto px-4 py-16">
            ...
            <div className="max-w-7xl mx-auto">
              <div className={`rounded-3xl shadow-xl overflow-hidden border transition-all duration-300 hover:shadow-2xl ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`} style={{  borderColor: darkMode ? '#374151' : `${color1}33` }} onMouseEnter={(e) => e.currentTarget.style.borderColor = `${color1}66`} onMouseLeave={(e) => e.currentTarget.style.borderColor = darkMode ? '#374151' : `${color1}33`}>
                <div className="p-8 md:p-12 lg:p-16">
                  <h2 className={`text-4xl md:text-5xl font-bold mb-12 text-center ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {governanceDocs.map((doc, index) => (
                      <a
                        ...
                        className={`group relative rounded-2xl p-6 border-2 hover:shadow-xl transition-all duration-500 hover:-translate-y-2 cursor-pointer overflow-hidden ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-200'}`}
                      ...
                      >
                        ...
                        <div className="relative flex items-center gap-4">
                          <div className="shrink-0 w-14 h-14 rounded-xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-all duration-300" style={{ backgroundColor: darkMode ? '#1f2937' : color2 }}>
                            {React.cloneElement(doc.icon, { className: "w-7 h-7", style: { color: color1 } })}
                          </div>
                          <div className="flex-1">
                            <h3 className={`text-lg font-bold transition-colors duration-300 leading-tight ${darkMode ? 'text-gray-100' : 'text-gray-900'}`} onMouseEnter={(e) => e.currentTarget.style.color = color1} onMouseLeave={(e) => e.currentTarget.style.color = 'inherit'}>
                              {doc.title}
                            </h3>
                          </div>
                        </div>
          ...
          <section className="mb-24">
            <div className="max-w-7xl mx-auto">
              <div className={`rounded-3xl p-10 md:p-16 shadow-xl overflow-hidden relative border-2 transition-all duration-500 cursor-pointer ${darkMode ? 'bg-gray-800' : ''}`} style={{  borderColor: darkMode ? '#374151' : `${color1}33`, backgroundColor: darkMode ? '' : color2 }} onMouseEnter={(e) => e.currentTarget.style.borderColor = `${color1}66`} onMouseLeave={(e) => e.currentTarget.style.borderColor = darkMode ? '#374151' : `${color1}33`}>
                ...
                <div className="relative text-center">
                  ...
                  <h2 className={`text-4xl md:text-5xl font-bold mb-12 ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                    Annual Reports
                  </h2>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {annualReports.map((report, i) => (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`flex items-center justify-center gap-3 backdrop-blur-sm px-4 py-4 rounded-xl border-2 hover:scale-105 transition-all duration-300 cursor-pointer shadow-sm hover:shadow-lg ${darkMode ? 'bg-gray-700' : 'bg-white'}`}
                      ...
                      >
                        <Download className="w-5 h-5 shrink-0" style={{ color: color1 }} />
                        <span className={`text-sm font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>{report.title}</span>
                      </a>
                    ))}
                  </div>
                </div>
