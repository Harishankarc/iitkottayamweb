...
import { Brain, Users, Mail, Target, Heart, Lightbulb, Camera, AlertCircle, Loader } from 'lucide-react';
...
export default function MindQuest() {
  ...
  const fetchContent = () => {
    setLoading(true);
    setError(null);
    API.get('/api/content-blocks/page/mind-quest')
      .then((response) => {
  ...
  const aboutBlock = contentBlocks.find(b => b.blockId === 'mindquest-about');
  const goalsBlock = contentBlocks.find(b => b.blockId === 'mindquest-goals');
  const coordinatorsBlock = contentBlocks.find(b => b.blockId === 'mindquest-coordinators');
  const membersBlock = contentBlocks.find(b => b.blockId === 'mindquest-members');
      ...
      <div className={`py-2 px-6 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto text-center">
          ...
          <h1 className={`text-2xl md:text-3xl font-bold mb-3 ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
            {heroBlock?.content?.title || 'Mind Quest'}
          </h1>
          <p className={`text-xs md:text-sm max-w-2xl mx-auto ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            {heroBlock?.content?.description || 'Promoting Mental Health & Well-being in Campus'}
          </p>
        </div>
      </div>
          ...
          {aboutBlock && (
            ...
            >
              <h2 className={`text-2xl font-bold mb-4 flex items-center gap-3 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                ...
                {aboutBlock.content?.title || 'About Mind Quest'}
              </h2>
          ...
          {goalsBlock && (
            <div>
              <h2 className={`text-2xl font-bold mb-6 flex items-center gap-3 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                <Heart size={28} style={{ color: API.color1 }} />
                {goalsBlock.content?.title || 'Our Goals'}
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {goalsBlock.content?.items?.map((item, index) => (
                  <div
                    key={index}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <Lightbulb size={20} style={{ color: API.color1, flexShrink: 0 }} />
