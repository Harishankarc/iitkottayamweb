          ...
          <a key={n.id || i} href={n.link || '/'} className={`block rounded-lg transition-all hover:shadow-md ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-white'}`}>
            <div className="flex gap-3 p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {n.pdfLink && (<div className="mt-2"><button onClick={(e) => {e.preventDefault(); e.stopPropagation(); setPdfModal(API.getImageUrl(n.pdfLink) || n.pdfLink);}} className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold text-white hover:opacity-90 transition" style={{ backgroundColor: color1 }}>PDF 📄</button></div>)}
              </div>
            </div>
          </a>
        ))}
