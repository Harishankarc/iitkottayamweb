...
export default function StudentMess() {
  ...
  const fetchContent = () => {
    setLoading(true);
    setError(null);
    API.get('/api/content-blocks/page/student-mess')
      .then((response) => {
      ...
      .catch((error) => {
        console.error('Error fetching mess content:', error);
        setError('Failed to load student mess content. Please try again later.');
        setContentBlocks([]);
      })
        ...
        <div key={block.id || index} className={`py-2 px-6 mb-6 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
          <div className="max-w-7xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium mb-3 border" 
              ...
              {block.content.badge || 'Dining & Nutrition'}
            </div>
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${color1}20` }}>
                    <Utensils className="w-4 h-4" style={{ color: color1 }} />
                  </div>
  ...
  const renderImageGroup = (blocks, index) => {
        ...
        <div key={`image-group-${index}`} className="mb-12 flex justify-center">
          ...
          >
            ...
            <img
              ...
              alt={block.content.alt || 'Mess facility'}
              className="w-full h-64 object-cover"
              onError={(e) => e.currentTarget.src = `https://placehold.co/600x400/${color1.replace('#', '')}/ffffff?text=Mess+Facility`}
            />
    ...
    return (
      ...
      >
        <h2 className="text-3xl font-bold mb-8 text-center" style={{ color: color1 }}>
          {blocks[0]?.content?.title || 'Mess Facilities Gallery'}
        </h2>
        <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
          {blocks.map((block, idx) => (
            ...
            >
              ...
              <img
                src={API.getImageUrl(block.content.url || block.content.src)}
                alt={block.content.alt || `Mess facility ${idx + 1}`}
                className="w-full h-64 object-cover"
                onError={(e) => e.currentTarget.src = `https://placehold.co/600x400/${color1.replace('#', '')}/ffffff?text=Mess+Facility`}
              />
              {block.content.caption && (
